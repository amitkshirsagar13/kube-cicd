...
def onlineKubernetes = new URL ("https://raw.githubusercontent.com/amitkshirsagar13/kube-cicd/master/jenkins/config/kubernetes/kubernetes.config").getText()

ConfigObject conf = new ConfigSlurper().parse(onlineKubernetes)
def kc
try {
...
//        if (podTemplateConfig.nodeUsageMode) podTemplate.setNodeUsageMode(podTemplateConfig.nodeUsageMode)
	...
	if (podTemplateConfig.keyValueEnvVar) { 
		def envVars = [] podTemplateConfig.keyValueEnvVar.each { 
			keyValueEnvVar ->
	                //since kubernetes-1.0
